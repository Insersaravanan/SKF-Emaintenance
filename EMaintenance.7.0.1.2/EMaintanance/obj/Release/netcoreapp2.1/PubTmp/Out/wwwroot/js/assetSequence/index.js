...
app.controller('skfCtrl', function ($scope, $filter, uiGridConstants, $http, $uibModal, languageFactory, alertFactory,$rootScope,$translate, $timeout) {
    $scope.startIndex = 1;
    ...
    var _columns = [
        ...
        {
            name: 'AssetTypeCode', displayName: 'Asset Type Code', headerCellFilter: 'translate',cellClass: getCellClass, enableColumnResizing: true, width: "8%", minWidth: 150, aggregationHideLabel: false, aggregationType: uiGridConstants.aggregationTypes.count,
            footerCellTemplate: '<div class="ui-grid-cell-contents" >{{"Total Count"|translate}}: {{col.getAggregationValue() | number:0 }}</div>'
        },
        {
            name: 'AssetTypeName', displayName: 'Asset Type Name', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "12%", minWidth: 150, aggregationHideLabel: false
        },
        {
            name: 'AssetSequenceCode', displayName: 'Asset Sequence Code', headerCellFilter: 'translate',cellClass: getCellClass, enableColumnResizing: true, width: "8%", minWidth: 100, aggregationHideLabel: false
           
        },
        { name: 'AssetSequenceName', displayName: 'Asset Sequence Name', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "18%", minWidth: 150 },
        { name: 'Descriptions', displayName: 'Descriptions', headerCellFilter: 'translate',cellClass: getCellClass, enableColumnResizing: true, width: "20%", minWidth: 300 },
        {
        ...
        },
        {
            ...
            cellTemplate: '<div class="ui-grid-cell-contents">' +
                '<a ng-click="grid.appScope.editRow(row.entity)" <i class="fa fa-pencil-square-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"EditAssetSequence"|translate}} tooltip-class="customClass"></i></a>' +
    ...
    $scope.clearModal = function () {
        ...
        $scope.AssetSequence = {
            ...
            AssetSequenceCode: null,
            AssetSequenceName: null,
            Descriptions: null,
            Active: null,
            AssetTypeId: 0
        };
    ...
    $scope.loadAssetType = function () {
        var _url = "/AssetType/GetAssetTypeByStatus?lId=" + $scope.language.LanguageId + "&status=Y";
        $http.get(_url)
    ...
    $scope.gridOpts = {
        ...
        exporterExcelFilename: 'EMaintenance_AssetSequence.xlsx',
        exporterExcelSheetName: 'EMaintenance_AssetSequence',
    ...
    $scope.loadData = function () {
        $scope.S_AssetSequence.languageId = $scope.language.LanguageId;
        $scope.isPageLoad = true;
        $scope.gridOpts.data = [];
        var postUrl = "/AssetSequence/Search";
        $http.post(postUrl, JSON.stringify($scope.S_AssetSequence)).then(function (response) {
            if (response.data) {
    ...
    $scope.save = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            ...
            var postUrl = "/AssetSequence/Create";

            $http.post(postUrl, JSON.stringify($scope.AssetSequence)).then(function (response) {
                if (response.data) {
    ...
    $scope.update = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            $scope.isProcess = true;
            var postUrl = "/AssetSequence/Update";
            $http.post(postUrl, JSON.stringify($scope.AssetSequence)).then(function (response) {
                if (response.data) {
                    if (response.data.toString().indexOf("<!DOCTYPE html>") >= 0) {
                        alertFactory.setMessage({
    ...
    $scope.multiLanguage = function (row) {
        $http.get("/AssetSequence/GetTransAssetSequence?atId=" + row.AssetSequenceId)
            .then(function (response) {
    ...
    $scope.import = function () {
        $scope.AssetSequence = "AssetSequence";
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                params: function () {
                    return { "language": $scope.language, "templateName": $scope.AssetSequence };
                }
            }
        });
...
app.controller('skfMultiLanguageModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory,$timeout) {
    var _param = params;
    $scope.AssetSequenceName = params.row.AssetSequenceName;
    $scope.formatters = {};
    ...
    var _columns= [
        ...
        { name: 'AssetSequenceName', displayName: 'Asset Sequence Name', headerCellFilter: 'translate', enableColumnResizing: true, enableCellEdit: true, enableFiltering: false, minWidth: 150 },
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/AssetSequence/SaveMultilingual";
