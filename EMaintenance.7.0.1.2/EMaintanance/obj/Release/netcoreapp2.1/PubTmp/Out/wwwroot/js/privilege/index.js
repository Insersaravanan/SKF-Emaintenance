...
app.controller('skfCtrl', function ($scope, $filter, uiGridConstants, $rootScope, $translate, $http, $uibModal, languageFactory, alertFactory,$timeout) {
    $scope.startIndex = 1;
    ...
    var _columns = [
        ...
        {
            name: 'PrivilegeCode', displayName: 'Privilege Code', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "30%", minWidth: 150, aggregationHideLabel: false, aggregationType: uiGridConstants.aggregationTypes.count,
            footerCellTemplate: '<div class="ui-grid-cell-contents" >{{"Total Count"|translate}}: {{col.getAggregationValue() | number:0 }}</div>'
        },
        { name: 'PrivilegeName', displayName: 'Privilege Name', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "30%", minWidth: 150 },
        {
        ...
        },
        {
            ...
            cellTemplate: '<div class="ui-grid-cell-contents">' +
                '<a ng-click="grid.appScope.editRow(row.entity)" <i class="fa fa-pencil-square-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"EditPrivilege"|translate}} tooltip-class="customClass"></i></a>' +
                '</div>',
            width: "14%",
            minWidth: 100
        }
    ...
    $scope.gridOpts = {
        ...
        exporterExcelFilename: 'EMaintenance_Privilege.xlsx',
        exporterExcelSheetName: 'EMaintenance_Privilege',
    ...
    $scope.loadData = function () {
        ...
        var postUrl = "/Privilege/Search";
        $http.post(postUrl, JSON.stringify($scope.S_Privilege)).then(function (response) {
            if (response.data) {
    ...
    $scope.save = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            ...
            var postUrl = "/Privilege/Create";

            $http.post(postUrl, JSON.stringify($scope.Privilege)).then(function (response) {
                if (response.data) {
