...
app.controller('skfCtrl', function ($scope, $filter, uiGridConstants, $http, $uibModal,$rootScope,$translate, languageFactory, alertFactory,$timeout) {
    $scope.startIndex = 1;
    ...
    var _columns = [
        ...
        {
            name: 'AssetClassCode', displayName: 'Asset Class Code', headerCellFilter: 'translate', cellClass: getCellClass,enableColumnResizing: true, width: "15%", minWidth: 150, aggregationHideLabel: false, aggregationType: uiGridConstants.aggregationTypes.count,
            footerCellTemplate: '<div class="ui-grid-cell-contents" >{{"Total Count"|translate}}: {{col.getAggregationValue() | number:0 }}</div>'
        },
        { name: 'AssetClassName', displayName: 'Asset Class Name', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "25%", minWidth: 150 },
        ...
        },
        {
            ...
            cellTemplate: '<div class="ui-grid-cell-contents">' +
                '<a ng-click="grid.appScope.editRow(row.entity)" <i class="fa fa-pencil-square-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"EditAssetClass"|translate}} tooltip-class="customClass"></i></a>' +
                ...
                '<a ng-click="grid.appScope.IndustryRel(row.entity)" <i class="fa fa-link icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"AssetClass-IndustryRelation"|translate}} tooltip-class="customClass"></i></a>' +
                '</div>',
            width: "11%",
            minWidth: 100
        }
    ...
    $scope.gridOpts = {
        ...
        exporterExcelFilename: 'EMaintenance_AssetClass.xlsx',
        exporterExcelSheetName: 'EMaintenance_AssetClass',
    ...
    $scope.loadData = function () {
        $scope.S_AssetClass.languageId = $scope.language.LanguageId;
        $scope.gridOpts.data = [];
        $scope.isPageLoad = true;
        var postUrl = "/AssetClass/Search";
        $http.post(postUrl, JSON.stringify($scope.S_AssetClass)).then(function (response) {
            if (response.data) {
    ...
    $scope.save = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            ...
            var postUrl = "/AssetClass/Create";

            $http.post(postUrl, JSON.stringify($scope.AssetClass)).then(function (response) {
                if (response.data) {
    ...
    $scope.multiLanguage = function (row) {
        $http.get("/AssetClass/GetTransAssetClass?atId=" + row.AssetClassId)
    ...
    $scope.import = function () {
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                params: function () {
                    return { "language": $scope.language, "templateName": $scope.AssetClass };
                }
            }
        });
    ...
    $scope.IndustryRel = function (row) {
        var modalInstance = $uibModal.open({
            templateUrl: 'skfAssetClassTypeRelModal.html',
            controller: 'skfAssetTypeClassRelModalCtrl',
            size: 'lg',
            backdrop: 'static',
            keyboard: false,
            resolve: {
...
app.controller('skfMultiLanguageModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory, $timeout) {
    ...
    var _columns = [
        ...
        {
            name: 'AssetClassName', displayName: 'Asset Class Name',headerCellFilter: 'translate',enableColumnResizing: true, enableCellEdit: true, enableFiltering: false, minWidth: 150 },
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/AssetClass/SaveMultilingual";
...
app.controller('skfAssetTypeClassRelModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory, $timeout) {
    var _param = params;
    $scope.AssetClassName = params.row.AssetClassName;
    $scope.formatters = {};

    $scope.LoadData = function () {
        $http.get("/AssetClass/GetAssetClassIndrel?lId=" + params.languageId + "&AssetClassId=" + params.row.AssetClassId)
            .then(function (response) {
    ...
    var _columns = [
        ...
        {
            name: 'SegmentName', displayName: 'Segment', headerCellFilter: 'translate', enableCellEdit: false, enableFiltering: true, minWidth: 150
        },
        {
            name: 'IndustryName', displayName: 'Industry', headerCellFilter: 'translate', enableCellEdit: false, enableFiltering: true, minWidth: 150
        },
        {
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/AssetClass/SaveAssetClassByIndustryRel";
            $http.post(postUrl, JSON.stringify(postData)).then(function (response) {
                if (response.status) {
                    if (response.data.toString().indexOf("<!DOCTYPE html>") >= 0) {
                        alertFactory.setMessage({
