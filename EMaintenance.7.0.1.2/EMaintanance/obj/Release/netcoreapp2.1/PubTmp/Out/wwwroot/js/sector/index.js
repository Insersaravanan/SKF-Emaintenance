...
app.controller('skfCtrl', function ($scope, $filter, uiGridConstants, $http, $uibModal, languageFactory, $rootScope,$translate,alertFactory, $timeout) {
    ...
    var _columns = [
        ...
        {
            name: 'SectorCode', displayName: 'Sector Code', headerCellFilter: 'translate',cellClass: getCellClass,enableColumnResizing: true, width: "15%", minWidth: 150, aggregationHideLabel: false, aggregationType: uiGridConstants.aggregationTypes.count,
            footerCellTemplate: '<div class="ui-grid-cell-contents" >{{"Total Count"|translate}}: {{col.getAggregationValue() | number:0 }}</div>'
        },
        { name: 'SectorName', displayName: 'Sector Name', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "25%", minWidth: 150 },
        ...
        },
        {
            ...
            cellTemplate: '<div class="ui-grid-cell-contents">' +
                '<a ng-click="grid.appScope.editRow(row.entity)" <i class="fa fa-pencil-square-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"EditSector"|translate}} tooltip-class="customClass"></i></a>' +
    ...
    $scope.gridOpts = {
        ...
        exporterExcelFilename: 'EMaintenance_Sector.xlsx',
        exporterExcelSheetName: 'EMaintenance_Sector',
    ...
    var _url = "/Sector/GetSectorByStatus?lId=" + $scope.language.LanguageId + "&status=" + $scope.S_Active;
        $http.get(_url)
    ...
    $scope.save = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            ...
            var postUrl = "/Sector/Create";

            $http.post(postUrl, JSON.stringify($scope.Sector)).then(function (response) {
                if (response.data) {
    ...
    $scope.multiLanguage = function (row) {
        $http.get("/Sector/GetTransSector?sId=" + row.SectorId)
            .then(function (response) {
    ...
    $scope.import = function () {
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                params: function () {
                    return { "language": $scope.language, "templateName": $scope.sector };
                }
            }
        });
...
app.controller('skfMultiLanguageModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory, $timeout) {
    var _param = params;
    $scope.sectorName = params.row.SectorName;
    $scope.formatters = {};
    ...
    var _columns = [
        ...
        { name: 'SectorName', displayName: 'Sector Name', headerCellFilter: 'translate',enableColumnResizing: true, enableCellEdit: true, enableFiltering: false, minWidth: 150 },
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/Sector/SaveMultilingual";
