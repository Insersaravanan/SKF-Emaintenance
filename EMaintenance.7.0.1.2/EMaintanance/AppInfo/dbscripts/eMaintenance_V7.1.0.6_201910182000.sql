...
-- Script date 18-10-19 20:00:52
-- Server version: 14.00.3048
-- Run this script against EmaintenancePROD to synchronize it with EmaintenanceUAT
-- 
...
--
-- Create column [SysRemarks] on table [dbo].[JobEquipUnitSymptoms]
--
ALTER TABLE [dbo].[JobEquipUnitSymptoms]
  ADD [SysRemarks] [varchar](max) NULL
GO
...
DECLARE GetJobUnitSymptomsCur CURSOR READ_ONLY
    ...
    WHILE @@FETCH_STATUS = 0
		BEGIN 
			if isnull(@UnitSymptomsId,0) = 0
			Begin
 				UPDATE [JobEquipUnitSymptoms] SET ACTIVE = 'N' , SysRemarks = concat('Inactivated since data are copied from previous history, replaced on  ',getdate())
				WHERE UnitAnalysisId = @UnitAnalysisId and  SymptomTypeId = @SymptomTypeId and FrequencyId = @FrequencyId
			End
...
BEGIN
	...
	select @FrequencyName =case when @ServiceType = 'OA' then 'JobFrequencyOA' else 'JobFrequency' End ,
	 @SymtomName = case when @ServiceType = 'OA' then 'JobSymptomTypeOA' when (@UnitType = 'DR' and @ServiceType = 'VA') 
	 then 'JobSymptomType' when (@UnitType = 'DN' and @ServiceType = 'VA') then 'JobSymptomType' when (@UnitType = 'IN' and @ServiceType = 'VA') then 'JobSymptomTypeShaft' End ;
	...
	from lookups a cross join lookups b where a.lname = @FrequencyName and b.lname = @SymtomName
 	...
 	Select u.UnitAnalysisId,u.JobEquipmentId,u.ServiceId,u.UnitType,u.UnitId, u.ConditionId,u.ConfidentFactorId,
	u.FailureProbFactorId,u.PriorityId,u.IsWorkNotification,u.NoOfDays,u.Recommendation,u.Comment,u.StatusId,
	(select isnull(js.UnitSymptomsId ,0) as UnitSymptomsId,isnull(js.UnitAnalysisId ,u.UnitAnalysisId) as UnitAnalysisId,
	isnull(js.SymptomTypeId,sl.SymptomTypeId) as SymptomTypeId,  
	dbo.GetLookupTranslated(isnull(js.SymptomTypeId,sl.SymptomTypeId), @LanguageId,'LookupValue')as SymptomType,
	isnull(js.FrequencyId,sl.FrequencyId) as FrequencyId,   
	dbo.GetLookupTranslated(isnull(js.FrequencyId,sl.FrequencyId), @LanguageId,'LookupValue')as Frequency,
	js.FailureModeId as FailureModeId,js.IndicatedFaultId as IndicatedFaultId,  
	js.Symptoms as Symptoms,isnull(js.Active,'Y') as Active
	...
	FOR JSON AUTO , INCLUDE_NULL_VALUES
			...
			( 	Select ua.UnitAnalysisId as UnitAnalysisId,ua.JobEquipmentId as JobEquipmentId,ua.ServiceId as ServiceId,ua.UnitType as UnitType,ua.UnitId, ua.ConditionId,ua.ConfidentFactorId,
					ua.FailureProbFactorId as FailureProbFactorId,ua.PriorityId as PriorityId,ua.IsWorkNotification as IsWorkNotification, ua.Recommendation as Recommendation,ua.Comment 
				from JobEquipUnitAnalysis ua
				where ua.UnitAnalysisId = isnull(dbo.GetAssetPreviousCondition(u.UnitType,u.UnitId,u.ServiceId,u.JobEquipmentId ),0)
				FOR JSON AUTO , INCLUDE_NULL_VALUES
				) JobPrevConditionJson, 
			(	select 0 as UnitSymptomsId,isnull(u.UnitAnalysisId ,0) as UnitAnalysisId,
				isnull(js.SymptomTypeId,sl.SymptomTypeId) as SymptomTypeId,  
				dbo.GetLookupTranslated(isnull(js.SymptomTypeId,sl.SymptomTypeId), @LanguageId,'LookupValue')as SymptomType,
				isnull(js.FrequencyId,sl.FrequencyId) as FrequencyId,   
				dbo.GetLookupTranslated(isnull(js.FrequencyId,sl.FrequencyId), @LanguageId,'LookupValue')as Frequency,
				...
				js.Symptoms as Symptoms,isnull(js.Active,'Y') as Active
				from JobEquipUnitSymptoms js Right Join @SymptomsLookup sl on (sl.SymptomTypeId = js.SymptomTypeId and sl.FrequencyId = js.FrequencyId) 
				and js.UnitAnalysisId = isnull(dbo.GetAssetPreviousCondition(u.UnitType,u.UnitId,u.ServiceId,u.JobEquipmentId ),0)
				and js.Active = 'Y'
				order by sl.SListOrder,sl.FListOrder				  
				FOR JSON AUTO , INCLUDE_NULL_VALUES
				) JobPrevUnitSymptomsListJson, 
	...
	isnull(ja.Active,'Y') as Active
	from JobEquipUnitAmplitude ja where ja.UnitAnalysisId = u.UnitAnalysisId 
	and ja.Active = 'Y'
	FOR JSON AUTO  , INCLUDE_NULL_VALUES
	) JobUnitAmplitudeListJson
				,
				(select 0 as UnitAmplitudeId, u.UnitAnalysisId   as UnitAnalysisId,
				ja.OAVibration  as OAVibration,
				ja.OAGELevelPkPk  as OAGELevelPkPk, ja.OASensorDirection as OASensorDirection,
				ja.OASensorLocation  as OASensorLocation, ja.OAVibChangePercentage  as OAVibChangePercentage,
				isnull(ja.Active,'Y') as Active
				from JobEquipUnitAmplitude ja where ja.UnitAnalysisId = dbo.GetAssetPreviousCondition(u.UnitType,u.UnitId,u.ServiceId,u.JobEquipmentId )
				and ja.Active = 'Y'
				FOR JSON AUTO  , INCLUDE_NULL_VALUES
				) JobPrevUnitAmplitudeListJson 
...
BEGIN  
 ...
 from JobEquipment where JobEquipmentId = @JobEquipmentId
	...
	select UnitAnalysisId,dbo.GetAssetPreviousCondition(UnitType,UnitId,ServiceId,JobEquipmentId ) as PreUnitAnalysisId, JobEquipmentId,@JobId as JobId,@EquipmentId as EquipmentId,
...
--
-- Alter procedure [dbo].[EAppDBActiveUserAudit]
--
GO
 
CREATE OR ALTER PROCEDURE [dbo].[EAppDBActiveUserAudit]
	@LanguageId int 
WITH ENCRYPTION
AS
BEGIN
	...
	where h.Active = 'Y' and h.CreatedOn > DATEADD(DAY,-1,getdate())
