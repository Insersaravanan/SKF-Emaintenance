...
namespace EMaintanance.Controllers
{
    ...
    public class LeverageController : Controller
    {
        ...
        [HttpGet]
        //COMMENTS BY SSK ON 20-03-2020 -LEVERAGED SERVIE STORED PROCEDURE ENCRPTED AND ALSO DISPLAY THE SELECTED CUSTOERM DATA
        public async Task<IActionResult> GetLeverage(DateTime? fromdate, DateTime? todate, int lId, int csId)
        {
            try
            {
                return Ok(await leverageRepo.GetLeverage(fromdate, todate, lId, csId));
            }
        ...
        public async Task<IActionResult> GetLeverageByCustomerID(DateTime? fromdate, DateTime? todate, int lId, int csId)
        {
            try
            {
                return Ok(await leverageRepo.GetLeverageByCustomerID(fromdate, todate, lId, csId));
            }
            catch (CustomException cex)
            {
                var responseObj = new EmaintenanceMessage(cex.Message, cex.Type, cex.IsException, cex.Exception?.ToString());
