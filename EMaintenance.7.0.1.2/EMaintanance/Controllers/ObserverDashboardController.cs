...
namespace EMaintanance.Controllers
{
    ...
    public class ObserverDashboardController : Controller
    {
        ...
        [HttpPost]
        //[SKFAuthorize("PRG48:P1")]
        public async Task<IActionResult> GetDashboardGoogleMapDetails([FromBody] ObserverDashboardViewModel obdvm)
        {
            CurrentUser cUser = new CurrentUser(HttpContext, _configuration);
            try
            {
                obdvm.UserId = cUser.UserId;
                var result = await observerDashboardRepo.GetDashboardGoogleMapDetails(obdvm);
                await auditLogService.LogActivity(cUser.UserId, cUser.HostIP, cUser.SessionId, "Area", "Area list Loaded.");
                return Ok(result);
            }
            catch (CustomException cex)
