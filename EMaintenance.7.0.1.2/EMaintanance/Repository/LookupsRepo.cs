...
namespace EMaintanance.Repository
{
    public class LookupsRepo
    {
        ...
        public async Task<IEnumerable<dynamic>> GetLookupByNameAndType(int LanguageId, string LName, string AType)
        {
            ...
            using (var conn = util.MasterCon())
            {
                try
                {
                    return await (conn.QueryAsync<dynamic>(sql, new { LanguageId, Status, LName, AType }, commandType: CommandType.StoredProcedure));
                }
                catch (Exception ex)
                {
                    throw new CustomException("Unable to Load Data, Please Contact Support!!!", "Error", true, ex);
