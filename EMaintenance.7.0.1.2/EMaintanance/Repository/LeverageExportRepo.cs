    ...
    public class LeverageExportRepo
    {
        ...
        public async Task<IEnumerable<dynamic>> GetLevergesByParams(LeverageExportSearchViewModel lsvm)
        {
            ...
            using (var conn = util.MasterCon())
            {
                try
                {
                    return await (conn.QueryAsync<dynamic>(sql, new { lsvm.LeverageFromDate, lsvm.LeverageToDate, lsvm.CountryId, lsvm.LanguageId, lsvm.ClientSiteId }, commandType: CommandType.StoredProcedure));
                }
                catch (Exception ex)
                {
                    throw new CustomException("Unable to Load Data, Please Contact Support!!!", "Error", true, ex);
        ...
        public async Task<IEnumerable<dynamic>> SaveExportLeverageFiles(LeverageExportViewModel lvm)
        {
            ...
            using (var conn = util.MasterCon())
            {
                try
                {
                    ...
                    return await (conn.QueryAsync<dynamic>(sql, new { LeverageServiceJson, lvm.FilePath, lvm.UserId, lvm.ClientSiteId }, commandType: CommandType.StoredProcedure));
                }
                catch (Exception ex)
                {
                    throw new CustomException("Unable to Save Or Update, Please Contact Support!!!", "Error", true, ex);
        ...
        public async Task<IEnumerable<dynamic>> GetExportLeverageFiles(LeverageExportSearchViewModel lsvm)
        {
            ...
            using (var conn = util.MasterCon())
            {
                try
                {
                    return await (conn.QueryAsync<dynamic>(sql, new { lsvm.FileFromDate, lsvm.FileToDate, lsvm.LanguageId, lsvm.ClientSiteId }, commandType: CommandType.StoredProcedure));
                }
                catch (Exception ex)
                {
                    throw new CustomException("Unable to Load Data, Please Contact Support!!!", "Error", true, ex);
