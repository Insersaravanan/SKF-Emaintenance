...
{
    public class FailureReportRepo
    {
        private readonly Utility util;

        public FailureReportRepo(IConfiguration configuration)
        {
        ...
        public async Task<IEnumerable<dynamic>> GetAttachmentById(int FailureReportHeaderId, string Status)
        {
            using (var conn = util.MasterCon())
            {
                try
                {
                    ...
                    return await (conn.QueryAsync<dynamic>(sql, new { FailureReportHeaderId, Status }, commandType: CommandType.StoredProcedure));
                }
                catch (Exception ex)
                {
                    throw new CustomException("Unable to Load Data, Please Contact Support!!!", "Error", true, ex);
