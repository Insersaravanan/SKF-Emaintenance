...
namespace EMaintanance.Repository
{
    public class ClientSiteRepo
    {
        ...
        public async Task<IEnumerable<dynamic>> GetClientSiteByProgramType(int LanguageId, int ProgramTypeId)
        {
            string sql = "dbo.EAppListClientSiteByProgramType";
            using (var conn = util.MasterCon())
            {
                try
                {
                    return await (conn.QueryAsync<dynamic>(sql, new { LanguageId, ProgramTypeId }, commandType: CommandType.StoredProcedure));
                }
                catch (Exception ex)
                {
                    throw new CustomException("Unable to Load Data, Please Contact Support!!!", "Error", true, ex);
