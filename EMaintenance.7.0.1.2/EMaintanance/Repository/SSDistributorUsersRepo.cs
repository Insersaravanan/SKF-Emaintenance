...
{
    public class SSDistributorUsersRepo
    {
        Utility util;
        ...
        public SSDistributorUsersRepo(IConfiguration iconfiguration)
        {
        ...
        {
            string sql = "Select UserName,UserId from dbo.SSDistributorUsers where UserName like '%" + UserName + "%' order by UserName";
        ...
        public async Task<IEnumerable<dynamic>> SaveOrUpdate(UsersViewModel u, UserManager<IdentityUser> identityManager)
        {
            ...
            if (appUser == null)
            {
                ...
                appConfig = await appConfigRepo.GetAppConfigByName("USER_ACTIVATION_ENABLED", "Y");
                IsUserActivationEnable = Boolean.Parse(appConfig.AppConfigValue);

                if (IsUserActivationEnable)
                {
                    appConfig = await appConfigRepo.GetAppConfigByName("USER_CONTROLLER_ACTION", "Y");
