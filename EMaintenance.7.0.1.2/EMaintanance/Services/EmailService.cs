...
namespace EMaintanance.Services
{
    public class EmailService
    {
        ...
        public async Task<dynamic> SendInvite(EmailViewModel emailViewModel)
        {
            ...
            try
            {
                appConfig = await appConfigRepo.GetAppConfigByName("SMTP_URL", "Y");
                SmtpUrl = appConfig.AppConfigValue;
                appConfig = await appConfigRepo.GetAppConfigByName("SMTP_UN", "Y");
                SmtpUN = appConfig.AppConfigValue;
                appConfig = await appConfigRepo.GetAppConfigByName("SMTP_PWD", "Y");
                SmtpPwd = CustomUtils.DecodeFromBase64(appConfig.AppConfigValue);
                appConfig = await appConfigRepo.GetAppConfigByName("SMTP_PORT", "Y");
                Port = Int32.Parse(appConfig.AppConfigValue);
                appConfig = await appConfigRepo.GetAppConfigByName("SENDER_EMAIL", "Y");
                SenderEmailId = appConfig.AppConfigValue;
                appConfig = await appConfigRepo.GetAppConfigByName("SMTP_ENABLE_SSL", "Y");
                ...
                ApplicationConfigurationViewModel sub = await appConfigRepo.GetAppConfigByName("APP_ENV", "Y");
