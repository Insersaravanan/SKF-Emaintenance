...
app.controller('skfCtrl', function ($scope, $filter, uiGridConstants, $http, $rootScope, $translate, $uibModal, languageFactory, alertFactory, clientFactory, $timeout) {
    $scope.startIndex = 1;
    $scope.isEdit = false;
    ...
    var _columns = [
        ...
        { name: 'ActualRepairCost', displayName: 'Actual Repair Cost ($)', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "16%", minWidth: 100 },
        { name: 'BreakDownCost', displayName: 'Break Down Cost ($)', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "16%", minWidth: 100 },
        { name: 'BreakDownHrs', displayName: 'Break Down Hrs', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "16%", minWidth: 100 },
        {
            name: 'ReportDate', displayName: 'ReportDate', headerCellFilter: 'translate',enableColumnResizing: true, enableCellEdit: false, minWidth: 80, width: "18%", cellFilter: 'date:\'dd/MM/yyyy\'', cellClass: getCellClass, aggregationType: uiGridConstants.aggregationTypes.count,
            footerCellTemplate: '<div class="ui-grid-cell-contents" >{{"Total Count"|translate}}: {{col.getAggregationValue() | number:0 }}</div>'
        },
        {
            name: 'CreatedBy', displayName: 'Report By', headerCellFilter: 'translate', enableColumnResizing: true, enableCellEdit: false, minWidth: 80, width: "18%", cellFilter: 'date:\'dd/MM/yyyy\'', cellClass: getCellClass
        },
        {
            ...
            cellTemplate: '<div class="ui-grid-cell-contents">' +
                '<a ng-click="grid.appScope.editRow(row.entity)" <i class="fa fa-pencil-square-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"EditFailureReport"|translate}} tooltip-class="customClass"></i></a>' +
                '<a ng-click="grid.appScope.FileUpload(row)"><i class="fa fa-file-picture-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"ViewAttachment"|translate}} tooltip-class="customClass"></i></a>' +
                '</div>',
            width: "13%",
            minWidth: 100
        }
    ...
    $scope.loadData = function () {
        ...
        var postUrl = "/FailureReport/GetFailureReportByStatus";
    ...
    $scope.FileUpload = function (row) {
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                params: function () {
                    return { "languageId": $scope.language.LanguageId, "aId": row.entity.FailureReportHeaderId, "Type": "FailureReport" };
                }
            }
        });
...
app.controller('skfFailureReportCtrl', function ($scope, $http, $filter, $uibModal, $uibModalInstance, params, uiGridConstants, alertFactory, $timeout) {
    ...
    if (params.isEdit) {
        ...
        $scope.EquipmentId = params.row.EquipmentId;
    ...
    $scope.update = function () {
        ...
        $scope.isProcess = true;
        var postUrl = "/FailureReport/Update";

        if ($scope.isDriveValidate && $scope.isInterValidate && $scope.isDrivenValidate) {
            $http.post(postUrl, JSON.stringify(_postdata)).then(function (response) {
                if (response.data) {
                    if (response.data.toString().indexOf("<!DOCTYPE html>") >= 0) {
                        alertFactory.setMessage({
