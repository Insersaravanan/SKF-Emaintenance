...
app.controller('skfCtrl', function ($scope, $filter, uiGridConstants, $http, $uibModal, $translate,$rootScope,languageFactory, alertFactory, $timeout) {
    $scope.startIndex = 1;
    ...
    var _columns = [
        {
            name: 'sno', displayName: '#', width: "4%", cellClass: getCellClass, minWidth: 50, enableFiltering: false, enableSorting: false, enableHiding: false,
            cellTemplate: '<div class="ui-grid-cell-contents">{{grid.renderContainers.body.visibleRowCache.indexOf(row) + 1}}</div>'
        },
        {
            name: 'AssetTypeCode', displayName: 'Asset Type Code', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "20%", minWidth: 150, aggregationHideLabel: false, aggregationType: uiGridConstants.aggregationTypes.count,
            footerCellTemplate: '<div class="ui-grid-cell-contents" >{{"Total Count"|translate}}: {{col.getAggregationValue() | number:0 }}</div>'
        },
        { name: 'AssetTypeName', displayName: 'Asset Type Name', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "20%", minWidth: 150 },
        { name: 'Descriptions', displayName: 'Descriptions', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "30%", minWidth: 300 },
        {
        ...
        },
        {
            ...
            cellTemplate: '<div class="ui-grid-cell-contents">' +
                '<a ng-click="grid.appScope.editRow(row.entity)" <i class="fa fa-pencil-square-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"EditAssetType"|translate}} tooltip-class="customClass"></i></a>' +
                ...
                '<a ng-click="grid.appScope.IndustryRel(row.entity)" <i class="fa fa-link icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"AssetType-AssetClassRelation"|translate}}} tooltip-class="customClass"></i></a>' +
                '</div>',
            width: "9%",
            minWidth: 100
        }
    ...
    $scope.gridOpts = {
        ...
        exporterExcelFilename: 'EMaintenance_AssetType.xlsx',
        exporterExcelSheetName: 'EMaintenance_AssetType',
    ...
    $scope.loadData = function () {
        $scope.S_AssetType.languageId = $scope.language.LanguageId;
        $scope.gridOpts.data = [];
        $scope.isPageLoad = true;
        var postUrl = "/AssetType/Search";
        $http.post(postUrl, JSON.stringify($scope.S_AssetType)).then(function (response) {
            if (response.data) {
    ...
    $scope.save = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            ...
            var postUrl = "/AssetType/Create";

            $http.post(postUrl, JSON.stringify($scope.AssetType)).then(function (response) {
                if (response.data) {
    ...
    $scope.multiLanguage = function (row) {
        $http.get("/AssetType/GetTransAssetType?atId=" + row.AssetTypeId)
    ...
    $scope.import = function () {
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                params: function () {
                    return { "language": $scope.language, "templateName": $scope.AssetType };
                }
            }
        });
    ...
    $scope.IndustryRel = function (row) {
        var modalInstance = $uibModal.open({
            templateUrl: 'skfIndustryRelModal.html',
            controller: 'skfIndustryRelModalCtrl',
            size: 'lg',
            backdrop: 'static',
            keyboard: false,
            resolve: {
...
app.controller('skfMultiLanguageModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory,$timeout) {
    var _param = params;
    $scope.assetTypeName = params.row.AssetTypeName;
    $scope.formatters = {};
    ...
    var _columns = [
        ...
        { name: 'AssetTypeName', displayName: 'Asset Type Name', headerCellFilter: 'translate', enableColumnResizing: true, enableCellEdit: true, enableFiltering: false, minWidth: 150 },
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/AssetType/SaveMultilingual";
...
app.controller('skfIndustryRelModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory, $timeout) {
    var _param = params;
    $scope.assetTypeName = params.row.AssetTypeName;
    $scope.formatters = {};

    $scope.LoadData = function () {
        $http.get("/AssetType/GetAssetTypeClass?LanguageId=" + params.languageId + "&AssetTypeId=" + params.row.AssetTypeId)
            .then(function (response) {
    ...
    var _columns = [
        ...
        },
        {
            ...
            headerCellTemplate: '<label class="ui-grid-cell-contents"><span>Select &nbsp;&nbsp</span><input type="checkbox" class="header-checkbox" ng-click="grid.appScope.SelectAll()" ng-true-value="\'Y\'" ng-false-value="\'N\'"></label>',
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/AssetType/SaveAssetTypeClass";
            $http.post(postUrl, JSON.stringify(postData)).then(function (response) {
                if (response.status) {
                    if (response.data.toString().indexOf("<!DOCTYPE html>") >= 0) {
                        alertFactory.setMessage({
