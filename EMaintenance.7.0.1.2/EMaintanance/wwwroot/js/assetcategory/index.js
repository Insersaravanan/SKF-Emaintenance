...
app.controller('skfCtrl', function ($scope, $filter, uiGridConstants, $http, $uibModal, languageFactory, $rootScope, $translate, alertFactory, $timeout) {
    $scope.startIndex = 1;
    ...
    var _columns= [
        ...
        {
            name: 'AssetCategoryCode', displayName: 'Asset Category Code', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "15%", minWidth: 150, aggregationHideLabel: false, aggregationType: uiGridConstants.aggregationTypes.count,
            footerCellTemplate: '<div class="ui-grid-cell-contents" >{{"Total Count"|translate}} {{col.getAggregationValue() | number:0 }}</div>'
        },
        { name: 'AssetCategoryName', displayName: 'Asset Category Name', headerCellFilter: 'translate',cellClass: getCellClass, enableColumnResizing: true, width: "25%", minWidth: 150 },
        { name: 'Descriptions', displayName: 'Descriptions', headerCellFilter: 'translate', cellClass: getCellClass, enableColumnResizing: true, width: "33%", minWidth: 300 },
        {
        ...
        },
        {
            ...
            cellTemplate: '<div class="ui-grid-cell-contents">' +
                '<a ng-click="grid.appScope.editRow(row.entity)" <i class="fa fa-pencil-square-o icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"EditAssetCategory"|translate}} tooltip-class="customClass"></i></a>' +
                ...
                '<a ng-click="grid.appScope.IndustryRel(row.entity)" <i class="fa fa-link icon-space-before" tooltip-append-to-body="true" uib-tooltip={{"AssetCategory-AssetClassRelation"|translate}} tooltip-class="customClass"></i></a>' +
                '</div>',
            width: "11%",
            minWidth: 100
        }
    ...
    $scope.gridOpts = {
        ...
        exporterExcelFilename: 'EMaintenance_AssetCategory.xlsx',
        exporterExcelSheetName: 'EMaintenance_AssetCategory',
    ...
    $scope.loadData = function () {
        $scope.S_AssetCategory.languageId = $scope.language.LanguageId;
        $scope.gridOpts.data = [];
        $scope.isPageLoad = true;
        var postUrl = "/AssetCategory/Search";
        $http.post(postUrl, JSON.stringify($scope.S_AssetCategory)).then(function (response) {
            if (response.data) {
    ...
    $scope.save = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            ...
            var postUrl = "/AssetCategory/Create";

            $http.post(postUrl, JSON.stringify($scope.AssetCategory)).then(function (response) {
                if (response.data) {
    ...
    $scope.update = function () {
        if ($scope.userForm.$valid && !($scope.isProcess) && !($scope.readOnlyPage)) {
            $scope.isProcess = true;
            var postUrl = "/AssetCategory/Update";
            $http.post(postUrl, JSON.stringify($scope.AssetCategory)).then(function (response) {
                if (response.data) {
                    if (response.data.toString().indexOf("<!DOCTYPE html>") >= 0) {
                        alertFactory.setMessage({
    ...
    $scope.multiLanguage = function (row) {
        $http.get("/AssetCategory/GetTransAssetCategory?atId=" + row.AssetCategoryId)
    ...
    $scope.import = function () {
        ...
        var modalInstance = $uibModal.open({
            ...
            resolve: {
                params: function () {
                    return { "language": $scope.language, "templateName": $scope.AssetCategory };
                }
            }
        });
    ...
    $scope.IndustryRel = function (row) {
        var modalInstance = $uibModal.open({
            templateUrl: 'skfAssetCategoryClassRelModal.html',
            controller: 'skfAssetCategoryClassRelModalCtrl',
            size: 'lg',
            backdrop: 'static',
            keyboard: false,
            resolve: {
...
app.controller('skfMultiLanguageModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory, $timeout) {
    var _param = params;
    $scope.AssetClassName = params.row.AssetClassName;
    $scope.formatters = {};
    ...
    var _columns = [
        ...
        { name: 'AssetCategoryName', displayName: 'Asset Category Name', headerCellFilter: 'translate', enableColumnResizing: true, enableCellEdit: true, enableFiltering: false, minWidth: 150 },
        { name: 'Descriptions', displayName: 'Descriptions', headerCellFilter: 'translate', enableColumnResizing: true, enableCellEdit: true, enableFiltering: false, minWidth: 200 }
    ];
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/AssetCategory/SaveMultilingual";
...
app.controller('skfAssetCategoryClassRelModalCtrl', function ($scope, $http, $uibModalInstance, params, uiGridConstants, alertFactory, $timeout) {
    var _param = params;
    $scope.AssetCategoryName = params.row.AssetCategoryName;
    $scope.formatters = {};

    $scope.LoadData = function () {
        $http.get("/AssetCategory/GetCategoryClassRel?lId=" + params.languageId + "&AssetCategoryId=" + params.row.AssetCategoryId)
            .then(function (response) {
    ...
    var _columns = [
        ...
        {
            name: 'AssetClassName', displayName: 'Asset Class', headerCellFilter: 'translate', enableCellEdit: false, enableFiltering: true, minWidth: 150
        },
        {
   ...
   $scope.SelectAll = function () {
    ...
    $scope.save = function () {
        ...
        if (!$scope.isProcess) {
            $scope.isProcess = true;
            var postUrl = "/AssetCategory/SaveCategoryClassRel";
            $http.post(postUrl, JSON.stringify(postData)).then(function (response) {
                if (response.status) {
                    if (response.data.toString().indexOf("<!DOCTYPE html>") >= 0) {
                        alertFactory.setMessage({
